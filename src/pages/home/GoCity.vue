<template>
  <div class="lineTop">
    ...
    <div class="clusterList">
      <van-popup :show="showCenter" custom-class="center" position="center" @close="togglePopup">
        <div class="item item_2 lineBottom">
          选择商圈
        </div>
        <div class="item lineBottom" v-for="(i,index) in clusterList" :key="index" @click="goHome(i)">
          {{i.name}}
        </div>
      </van-popup>
    </div>
    ...
    <div class="box_1" v-if="ishowbox1">
      ...
      <div>
        ...
        <div v-for="(item,index) in showCity" :key="index" class="letter-item">
          ...
          <div v-for="(i,key) in item.citylist" :key="key" @click="getMasterList(i)">{{i}} </div>
        </div>
      </div>
    ...
    <div class="box_2" v-if="ishowbox2">
      ...
      <ul>
        ...
        <li class="f14 color32" :class="{'adr_name':address_name==i.name}" v-for="(i,index) in clusterList" :key="index"
          @click="goHome(i)">
...
export default {
  ...
  async onShow() {
    ...
    this.locationCity = this.$store.state.user.clusterInfo
