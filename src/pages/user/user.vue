<template>
  ...
  <div class="container">
    ...
    <div class="user_wrap">
      ...
      <div class="panel">
      	<div class="panelBox">
      		 <div class="updateUserInfo f13" v-if="hasAuthInfo" @click="$router.push({ path: '/pagesSub/userInfo/userInfo' })">
		        <img src="../../../static/images/icon_modify.png" alt="" />
		      </div>
      	</div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myFocus'})" >
          <div class="mt10 f12 color9a">{{authUserInfo?authUserInfo.statics.follow:0}}</div>
          <div class="mt1 f12 color32">关注</div>
        </div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myLike'})">
          <div class="mt10 f12 color9a">{{authUserInfo?authUserInfo.statics.favorite:0}}</div>
          <div class="mt1 f12 color32">收藏</div>
        </div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myFooter'})">
          <div class="mt10 f12 color9a">{{authUserInfo?authUserInfo.statics.history:0}}</div>
          <div class="mt1 f12 color32">足迹</div>
        </div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myPublic'})">
          <div class="mt10 f12 color9a">{{authUserInfo?authUserInfo.statics.publish:0}}</div>
          <div class="mt1 f12 color32">发布</div>
        </div>
      </div>
     

      <div class="myOrder lineBottom" @click="$router.push({ path: '/pagesSub/userInfo/myOrder'})">
        <span class="hander_name fl f16">我的订单</span>
        <span class="detail mr20 fr color9a f14">更多</span>
        <div class="arrow"></div>
      </div>
      <div class="orderStatus">
        <!-- <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 1 } })"> -->
        <!--mypay注释-->
        <!--<div class="item" @click="$router.push('myPay')">
                <img class="card" src="../../../static/images/icon_obligations.png">
                <div class="cardName">待付款</div>
            </div>-->
            <!-- order_0  待付款  order_1 待发货 order_2 待收货  order_4  售后中 -->
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 1 } })">
          <img class="card" src="../../../static/images/icon_obligations.png">
          <div class="cardName">待付款</div>
          <div class="right_radio" v-if="state.order_0 > 0">{{authUserInfo?authUserInfo.statics.order_0:''}}</div>
        </div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 2 } })">
          <img class="card" src="../../../static/images/icon_completed.png">
          <div class="cardName">待发货</div>
          <div class="right_radio" v-if="state.order_1 > 0">{{authUserInfo?authUserInfo.statics.order_1:''}}</div>
        </div>
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 3 } })">
        ...
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 4 } })">
        ...
        <div class="item" @click="$router.push({ path: '/pagesSub/userInfo/myOrder', query: { orderType: 5 } })">
    ...
    <div class="managementList">
      ...
      <div class="handerItem" @click="$router.push({ path: '/pagesSub/userInfo/myAdress'})">
        <div class="icon">
      ...
      <div class="handerItem" @click="$router.push({ path: '/pagesSub/userInfo/setting'})">
        <div class="icon">
  ...
  methods: {
    async hasauth() {
      ...
      if (
        ...
        this.$store.state.user.loginStatus == "wap"
      ) {
    ...
    goLogin_person() {
      if (this.hasAuthInfo) {
        this.$router.push({ path: "/pagesSub/userInfo/userInfo" });
      } else {
...
.container {
  ...
  .user_wrap {
    ...
    .bg {
      width: 100%;
      height: 110px;
      background: url("#{$imgUrl}img_user.png") no-repeat center center;
    ...
    .panel {
      ...
      box-shadow: 0rem -2px 3px 0px rgba(50, 50, 50, 0.1);
      border-radius: 5px;
      /*  border-radius: 5px;*/
      .panelBox {
        width: 345px;
