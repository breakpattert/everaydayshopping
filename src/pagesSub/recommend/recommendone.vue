  ...
  methods: {
    ...
    async initData() {
      ...
      if (this.$route.query.type == "info") {
        ...
        this.content = everyRecommendList.result.content;
      } else if (this.$route.query.type == "newFlash") {        
        let list = await this.$http.brandNews(this.$route.query.id);
				this.content = list.result.content;
				this.$http.readBrandNews(this.$route.query.id);
      } else {
        let [err, everyRecommendList] = await this._to(
