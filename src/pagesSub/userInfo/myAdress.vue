...
<div class="container lineTop" :class="{'iphonex':iPhoneshow}" :style="{height:iphoneheight,minHeight:iphoneheight}">
  ...
  <div :class="{'w90':getAdressList.length>0}" class="newAress f14" @click="$router.push({ path:'/pagesSub/userInfo/editAdress',query: { type: 'add' }})">
      新增地址
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    editAddr(id){
      console.log(id)
      this.$router.push({ path:'/pagesSub/userInfo/editAdress',query: { id: id , type:'edit'}})
    }
  },
