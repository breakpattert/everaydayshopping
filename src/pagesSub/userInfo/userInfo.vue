  ...
  <div class="container lineTop">
    <div class="userInfo">
      ...
      <div class="sex lineBottom">
        <picker @change="bindPickerChange" :value="gender-1" :range="array">
      ...
      <div class="brth lineBottom">
        <span>出生日期</span>
        <picker mode="date" :value="birthday" start="1900-09-01" end="2018-09-01" @change="bindDateChange">
          <p class="picker">
            <span> {{birthday}}</span>
          </p>
        </picker>
      ...
      <div class="userName lineBottom" @click="$router.push({ path:'/pagesSub/userInfo/userSignature',query:{signature}})">
        <span>个性签名</span>
        <div class="arrow"></div>
        <p class="over_hidden flex1 pl10" style="text-align: right;">{{signature?signature:'未设置'}}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    bindPickerChange(e) {
      this.gender = parseInt(e.mp.detail.value) + 1;
  ...
  async onShow() {
    ...
    this.gender = parseInt(res.gender);
