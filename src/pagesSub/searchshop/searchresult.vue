<template>
  <div class="container lineTop" @touchstart.prevent :class="{'iphonex':iPhoneshow}" :style="{height:iphoneheight,minHeight:iphoneheight}">
    ...
    <div class="w100 s_box lineTop lineBottom">
      <!--渐变-->
    ...
    <div class="swperlist" v-if="showresult" :class="{'swperlistTop':showresult}">
      ...
      <div id="swper1" v-if="activeIndex == 0">
        ...
        <div class="item" v-for="(item,index) in resultlist" :key="index" @click="$router.push({ path:'/pagesSub/busineshop/shop',query: { id: item.id}})">
          ...
          <div class="right">
            <div class="title f16 color32">
              ...
              <span class="f14 color32 over_hidden merchName">{{item.merchname}}</span>
              <span class="f12 color9a fr mt3">{{item.jl}}km</span>
            </div>

            <div class="time mb6 mt6" style="overflow: hidden;">
              <span class=" f12 ridio color9a">
                价位：
              </span>
              <span class="f12 color9a mr15">{{item.jiawei}}</span>
              <span class="f12 ridio color9a">
                热度：
              </span>
              <span class="f12 color9a">{{item.hot}}</span>
            </div>
        ...
        <div v-if="!resultlist||resultlist.length==0" class="empty4">
      ...
      <div id="swper2" v-if="activeIndex == 1">
        ...
        <div v-if="resultlist.length=='0'" class="empty4">
          <img src="../../../static/images/icon_news2.png">
...
export default {
  ...
  methods: {
    ...
    async sult(type) {
      // 搜索结果的页面
      let keywords = this.userword.replace(/\s+/g, "");
      let types = type;
      let clusterId = "";
      let cateid = "";
      this.isBottom = false;
...
.container {
  ...
  .result_page {
    ...
    /*position: fixed;
			top: 59px;*/
    box-shadow: 0 1px 1px 0 rgba(23, 34, 49, 0.1);
    padding: 0 15px;
    padding-top: 3px;
  ...
  .swperlist {
    ...
    #swper3 {
      ...
      .item {
        ...
        background: #fff;
        box-shadow: 0 1px 2px 0 rgba(154, 154, 154, 0.2);
        display: flex;
        .z_price {
          display: inline-block;
          position: absolute;
