    ...
    <div class="container lineTop">
        <scroll-view @scrolltolower='scrolltolower' lower-threshold='200' scroll-y class="likeAll" id="item1">
            <div class="w100 item-wrap"  v-if="list1.length>0">
                <div class="item flex flex_hc lineBottom" v-for="(item,index) in list1" :key="index" @click="toDetail(item.id,item.senderid)">
                    <img class="shopLogo" :src="item.avatar" alt="">
                    <div class="shopName flex1">
                        <p class="flex flex_hc over_hidden f14 color32">
                          <i class="bold flex1">
                              {{item.nickname}}
                          </i>
                          <span class="f10 color9a ml15">{{item.createtime}}</span></p>
                        <p class="f12 color9a mt5">
                           {{item.content}}
                        </p>
