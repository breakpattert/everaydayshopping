...
export default {
  ...
  methods: {
    ...
    async commentlist() {
      this.isBottom = false;
      let Data = await this.$http.askCommentList(
        this.articleid,
        this.openId,
        this.page,
        this.pagesize
      ...
      Data.comment_list.map(val => {
        val.replylist.map(this._U.hasOverTime);
    ...
    async setcomment() {
      if (this.$store.state.user.loginStatus == false) {
        ...
        if (this.setstatus == "直接评论") {
          let set = await this.$http.askAddComment(
            this.articleid,
            this.openId,
            this.setcontent
          );
    ...
    async setfloorReply() {
      ...
      let res = await this.$http.askFloorReply(
        ids.articleid,
        ids.commentid,
        ids.from_openid,
        ids.to_openid,
    ...
    async deleteComment(commentid) {
      let res = await this.$http.askDeleteComment(commentid, this.openId);
    ...
    async deleteReply(replyid) {
      let res = await this.$http.askDeleteReply(replyid, this.openId);
    ...
    async articlePraiseComment(commentid, islike) {
      if (this.$store.state.user.loginStatus == false) {
        ...
        let List = await this.$http.askPraiseComment(
          commentid,
          this.openId,
          this.articleid
        );
    ...
    async articlePraiseReply(commentid, islike, replyid) {
      if (this.$store.state.user.loginStatus == false) {
        ...
        let List = await this.$http.askPraiseReply(
          commentid,
          this.openId,
          replyid
        );
